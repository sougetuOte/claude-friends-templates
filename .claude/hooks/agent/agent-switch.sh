...
detect_agent_switch() {
    ...
    # Extract prompt from JSON with improved error handling
    if ! prompt=$(jq -r '.prompt // ""' "$prompt_file" 2>/dev/null); then
        _debug "Failed to parse JSON from: $prompt_file"
        echo "none"
        return 2
    fi

    # Validate agent commands with more precise regex matching
    if [[ "$prompt" =~ /agent:planner([[:space:]]|$) ]]; then
        ...
        # Invalid agent name - improved error reporting
        local -r invalid_agent="${BASH_REMATCH[1]}"
        _error "Invalid agent: $invalid_agent"
...
trigger_handover_generation() {
    ...
    # Generate handover file with improved error handling
    local -r handover_file="${CLAUDE_PROJECT_DIR:-$(pwd)}/.claude/${from_agent}/handover.md"
...
update_active_agent() {
    ...
    # Validate agent name format (security)
    if [[ ! "$new_agent" =~ ^[a-zA-Z0-9_-]+$ ]]; then
        _error "Invalid agent name format: $new_agent"
        return 1
    fi
...
initialize_agent_environment() {
    ...
    if [[ ! "$agent" =~ ^[a-zA-Z0-9_-]+$ ]]; then
        _error "Invalid agent name format: $agent"
        return 1
    fi

    local -r agent_dir="${CLAUDE_PROJECT_DIR:-$(pwd)}/.claude/${agent}"
    local -r claude_dir="${CLAUDE_PROJECT_DIR:-$(pwd)}/.claude"
...
main() {
    ...
    if [[ -f "$active_file" ]]; then
        current_agent=$(jq -r '.current_agent // "none"' "$active_file" 2>/dev/null) || {
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
    ...
    if ! init_hooks_system; then
        echo "[ERROR] Failed to initialize hooks system" >&2
        exit 1
    fi
