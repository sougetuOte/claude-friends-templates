...
class ParallelTaskAnalyzer:
    ...
    def _calculate_metrics(self) -> Dict[str, Any]:
        ...
        for phase_num, phase_tasks in enumerate(self.execution_plan):
            phase_times = [
                self.tasks[task_id].estimated_time for task_id in phase_tasks
            ]
            ...
            phase_details.append(
                {
                    ...
                    "duration": phase_time,
                    "bottleneck": max(
                        phase_tasks, key=lambda t: self.tasks[t].estimated_time
                    )
                    if phase_tasks
                    else None,
                }
        ...
        critical_path_time = sum(
            self.tasks[task_id].estimated_time for task_id in critical_path
        )
    ...
    def _find_critical_path(self) -> List[str]:
        ...
        try:
            ...
            sources = [
                n for n in weighted_graph.nodes() if weighted_graph.in_degree(n) == 0
            ]
            # Find all sink nodes (no outgoing edges)
            sinks = [
                n for n in weighted_graph.nodes() if weighted_graph.out_degree(n) == 0
            ]
            ...
            for source in sources:
                for sink in sinks:
                    try:
                        ...
                        for path in paths:
                            path_length = sum(
                                self.tasks[node].estimated_time for node in path
                            )
    ...
    def _format_execution_plan(self) -> List[Dict[str, Any]]:
        ...
        for phase_num, phase_tasks in enumerate(self.execution_plan):
            phase_info = {"phase": phase_num + 1, "parallel_tasks": []}
...
def main():
    ...
    parser.add_argument("input", help="Input tasks file (YAML or JSON)")
    parser.add_argument("-o", "--output", help="Output execution plan file")
    parser.add_argument(
        "-v", "--visualize", help="Generate visualization (provide output image path)"
    )
    parser.add_argument(
