...
class HandoverOptimizer:
    ...
    def _get_compression_rules(self) -> Dict[str, Dict[str, Any]]:
        ...
        if self.compression_level == "high":
            ...
            base_rules["remove_sections"] = ["background", "nice_to_have", "future_considerations"]
    ...
    def _optimize_section(self, section_name: str, content: str) -> Optional[str]:
        ...
        priority = self.compression_rules["section_priorities"].get(section_name, "medium")
        ...
        for line in lines:
            if re.match(r'^\s*[-*]\s+', line) or re.match(r'^\s*\d+\.\s+', line):
                if bullet_count < max_bullets:
                    ...
                    self.stats["removed_items"].append(f"Bullet point in {section_name}")
            else:
...
def main():
    ...
    parser.add_argument(
        "input",
        help="Input handover document path"
    )
    parser.add_argument(
        "-o", "--output",
        help="Output path (default: input_optimized.md)"
    )
    parser.add_argument(
        "-c", "--compression-level",
    ...
    if args.stats:
        ...
        print(f"  Compression ratio: {stats['optimized_size']/stats['original_size']:.2%}")
