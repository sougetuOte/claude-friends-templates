...
class CodeAnalyzer:
    ...
    def _analyze_python(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            result = subprocess.run(
                ["radon", "mi", file_path], capture_output=True, text=True, timeout=30
            )
            if result.returncode == 0:
        ...
        try:
            ...
            if result.stdout:
                ...
                if eslint_data and len(eslint_data) > 0:
                    ...
                    complexity_issues = [
                        m for m in messages if "complexity" in m.get("ruleId", "")
                    ]
    ...
    def _is_test_file(self, file_path: str) -> bool:
        ...
        return any(
            pattern in file_name for pattern in ["test", "spec", "_test", ".test"]
        )
...
class TestCoverageAnalyzer:
    ...
    def _detect_language(self, project_root: str) -> str:
        ...
        if (root_path / "package.json").exists():
            return "javascript"
        elif (root_path / "requirements.txt").exists() or (
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if coverage_file.exists():
                    ...
                    covered_lines = sum(
                        sum(1 for hits in file_data["l"].values() if hits > 0)
                        for file_data in coverage_data.values()
                        if "l" in file_data
                    )
    ...
    def _get_generic_coverage(self, project_root: str) -> Dict[str, Any]:
        ...
        return {
            "total_coverage": 0,
            "error": "Coverage analysis not supported for this language",
        }
...
class TDDCycleAnalyzer:
    ...
    def _calculate_cycle_metrics(
        ...
        coverage_delta = (
            final_state["overall_coverage"] - initial_state["overall_coverage"]
        )
        ...
        efficiency_score = self._calculate_efficiency_score(
            tracker, coverage_delta, complexity_delta
        )
        ...
        return TDDCycleMetrics(
            ...
            success=all(
                [
                    tracker.red_phase.success,
                    tracker.green_phase.success,
    ...
    def _calculate_complexity_delta(
        self, initial_state: Dict, final_state: Dict
    ) -> float:
    ...
    def _calculate_efficiency_score(
        self, tracker: "TDDCycleTracker", coverage_delta: float, complexity_delta: float
    ) -> float:
    ...
    def _calculate_phase_balance_score(self, tracker: "TDDCycleTracker") -> float:
        ...
        balance_score = (
            1.0 - (red_deviation + green_deviation + refactor_deviation) / 3.0
        )
    ...
    def _get_quality_score(self, state: Dict) -> float:
        ...
        for file_metrics in state["files"].values():
            ...
            comment_score = (
                ...
                else max(0, 1.0 - abs(comment_ratio - 0.2) * 5)
            )
            ...
            if file_metrics.get("test_cases_count", 0) > 0:
                ...
                test_score = min(
                    1.0, assertions_per_test / 3.0
                )  # Ideal: 3 assertions per test
    ...
    def _emit_efficiency_alert(
        self, recent_avg: float, older_avg: float, recent_cycles: List[TDDCycleMetrics]
    ):
    ...
    def _emit_phase_balance_alert(
        self, deviations: Dict, cycles: List[TDDCycleMetrics]
    ):
    ...
    def _generate_efficiency_recommendations(
        self, cycles: List[TDDCycleMetrics]
    ) -> List[str]:
        ...
        if long_cycles:
            recommendations.append(
                "Consider breaking down complex features into smaller TDD cycles"
            )
        ...
        if failed_cycles:
            recommendations.append(
                "Review and strengthen error handling in TDD process"
            )

        if not recommendations:
            recommendations.append(
                "Continue current TDD practices - efficiency is stable"
            )
    ...
    def _generate_phase_recommendations(self, deviations: Dict) -> List[str]:
        ...
        for phase, deviation_info in deviations.items():
            ...
            if actual > ideal:
                if phase == "red":
                    recommendations.append(
                        "Red phase taking too long - consider simpler test cases or better test planning"
                    )
                elif phase == "green":
                    recommendations.append(
                        "Green phase taking too long - consider simpler implementation approaches"
                    )
                elif phase == "refactor":
                    recommendations.append(
                        "Refactor phase taking too long - consider more frequent smaller refactorings"
                    )
            else:
                if phase == "red":
                    recommendations.append(
                        "Red phase too short - ensure comprehensive test scenarios"
                    )
                elif phase == "green":
                    recommendations.append(
                        "Green phase too short - ensure robust implementation"
                    )
                elif phase == "refactor":
                    recommendations.append(
                        "Refactor phase too short - allocate more time for code quality improvements"
                    )
    ...
    def generate_tdd_report(self, days: int = 7) -> Dict[str, Any]:
        ...
        recent_cycles = [
            cycle for cycle in self.cycle_history if cycle.start_time > cutoff_time
        ]
        ...
        avg_phase_durations = {
            ...
            "refactor": sum(c.refactor_phase.duration for c in recent_cycles)
            / total_cycles,
        }
        ...
        return {
            ...
            "phase_analysis": {
                "avg_durations_minutes": {
                    k: v / 60 for k, v in avg_phase_durations.items()
                },
                "phase_ratios": {
    ...
    def _calculate_efficiency_trend(self, cycles: List[TDDCycleMetrics]) -> str:
        ...
        second_half_avg = sum(c.efficiency_score for c in cycles[mid_point:]) / (
            len(cycles) - mid_point
        )
    ...
    def _calculate_coverage_trend(self, cycles: List[TDDCycleMetrics]) -> str:
        ...
        mid_point = len(cycles) // 2
        first_half_avg = (
            sum(c.test_coverage_final for c in cycles[:mid_point]) / mid_point
        )
        second_half_avg = sum(c.test_coverage_final for c in cycles[mid_point:]) / (
            len(cycles) - mid_point
        )
    ...
    def _generate_overall_recommendations(
        ...
        if avg_efficiency < 0.6:
            recommendations.append(
                "Focus on improving TDD cycle efficiency through better planning and simpler iterations"
            )

        if avg_coverage < 80:
            recommendations.append(
                "Aim for higher test coverage (target: 80%+) in each TDD cycle"
            )
        ...
        if len(long_cycles) > len(cycles) * 0.3:
            recommendations.append(
                "Break down features into smaller, more focused TDD cycles"
            )

        if not recommendations:
            recommendations.append(
                "Excellent TDD practices! Continue maintaining high standards"
            )
...
class TDDCycleTracker:
    ...
    def _track_phase(self, phase_name: str):
        ...
        phase_tracker = TDDPhaseTracker(
            phase_name, self.test_files + self.implementation_files, self.analyzer
        )
        yield phase_tracker
...
class TDDPhaseTracker:
    ...
    def __init__(self, phase_name: str, files: List[str], analyzer: TDDCycleAnalyzer):
        ...
        for file_path in files:
            if os.path.exists(file_path):
                self.initial_file_states[file_path] = (
                    self.analyzer.code_analyzer.analyze_file(file_path)
                )
...
def track_tdd_cycle(
    test_files: List[str], implementation_files: List[str], project_root: str = None
):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "command", choices=["analyze", "report", "demo"], help="Command to execute"
    )
