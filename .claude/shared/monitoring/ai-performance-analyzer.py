...
try:
    ...
    print("Warning: Some ML libraries not available. Install with: pip install numpy pandas scikit-learn")
    np = None
...
class BaselineCalculator:
    ...
    def is_anomalous(self, metric_name: str, value: float, threshold_std: float = 2.0) -> Tuple[bool, float]:
...
class AnomalyDetector:
    ...
    def prepare_features(self, metrics_data: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in numeric_columns:
            if col in ['value', 'duration_ms', 'memory_mb', 'cpu_percent']:
                ...
                # Rolling statistics
                features[f'{col}_rolling_mean_5'] = metrics_data[col].rolling(5, min_periods=1).mean()
                features[f'{col}_rolling_std_5'] = metrics_data[col].rolling(5, min_periods=1).std().fillna(0)
                features[f'{col}_rolling_max_10'] = metrics_data[col].rolling(10, min_periods=1).max()
    ...
    def train(self, metrics_data: pd.DataFrame, contamination: float = 0.1):
        ...
        print(f"Anomaly detection model trained on {len(features)} samples with {len(self.feature_columns)} features")
        return True
...
class PerformancePredictor:
    ...
    def _prepare_time_series_features(self, data: pd.Series, window_size: int = 10) -> pd.DataFrame:
    ...
    def train_predictor(self, metric_name: str, time_series_data: pd.Series,
                       test_size: float = 0.2) -> Dict[str, float]:
        """Train a predictor for a specific metric"""
        if len(time_series_data) < 50:
            print(f"Warning: Insufficient data for {metric_name} (need at least 50 samples)")
            return {}
        ...
        # Save model
        model_path = self.models_dir / f'predictor_{metric_name.replace(".", "_")}.joblib'
        joblib.dump({
            'model': model,
            'scaler': scaler,
    ...
    def predict(self, metric_name: str, historical_data: pd.Series,
                horizon: int = 12) -> Optional[PerformancePrediction]:
        ...
        last_complete_row = features.dropna().iloc[-1:] if len(features.dropna()) > 0 else features.iloc[-1:]
        ...
        for step in range(horizon):
            ...
            try:
                ...
                confidence_intervals.append((prediction - uncertainty, prediction + uncertainty))
        ...
        return PerformancePrediction(
            ...
            confidence_interval=confidence_intervals[0] if confidence_intervals else (0, 0),
            trend_analysis=trend_analysis,
            risk_assessment=risk_assessment
        )

    def _analyze_trends(self, predictions: List[float], historical_data: pd.Series) -> Dict[str, Any]:
        ...
        if len(historical_values) >= 2:
            hist_slope, _ = np.polyfit(np.arange(len(historical_values)), historical_values, 1)
        else:
            hist_slope = 0
        ...
        return {
            ...
            'trend_change': 'accelerating' if abs(slope) > abs(hist_slope) else 'decelerating',
            'predicted_change_percent': ((predictions[-1] - predictions[0]) / max(predictions[0], 0.001)) * 100
        }

    def _assess_risks(self, predictions: List[float], historical_data: pd.Series) -> Dict[str, Any]:
        ...
        if len(predictions) >= 2:
            max_change = max(abs(predictions[i+1] - predictions[i]) for i in range(len(predictions)-1))
            historical_max_change = max(abs(historical_data.diff().dropna()))

            if max_change > historical_max_change * 2:
                risks.append(f"Rapid change detected: {max_change:.2f} (historical max: {historical_max_change:.2f})")
...
class PatternAnalyzer:
    ...
    def find_correlations(self, metrics_data: pd.DataFrame, min_correlation: float = 0.7) -> List[PatternInsight]:
            ...
            for j, col2 in enumerate(numeric_cols[i+1:], i+1):
                ...
                if abs(correlation) >= min_correlation:
                    insight = PatternInsight(
                        ...
                        actionable_insights=self._generate_correlation_insights(col1, col2, correlation),
                        supporting_data={
    ...
    def _generate_correlation_insights(self, metric1: str, metric2: str, correlation: float) -> List[str]:
        ...
        if 'duration' in metric1 and 'memory' in metric2:
            if correlation > 0:
                insights.append("High execution time correlates with high memory usage - consider memory optimization")
            else:
                insights.append("Inverse correlation between time and memory suggests good memory management")

        elif 'hook' in metric1 and 'agent_switch' in metric2:
            if correlation > 0:
                insights.append("Hook performance affects agent switching - optimize critical hooks")

        elif 'energy' in metric1 and any(x in metric2 for x in ['cpu', 'memory', 'duration']):
            if correlation > 0:
                insights.append("Energy consumption correlates with resource usage - focus on efficiency")

        if not insights:
            insights.append(f"Monitor {metric1} and {metric2} together for optimization opportunities")

        return insights

    def detect_seasonal_patterns(self, time_series: pd.Series, metric_name: str) -> List[PatternInsight]:
        ...
        if isinstance(time_series.index, pd.DatetimeIndex):
            ...
            hourly_cv = hourly_data.std() / hourly_data.mean() if hourly_data.mean() > 0 else 0
            if hourly_cv > 0.2:  # Significant hourly variation
                ...
                insight = PatternInsight(
                    ...
                    actionable_insights=[
                        ...
                        f"Optimize resources for peak usage periods",
            ...
            weekly_cv = daily_data.std() / daily_data.mean() if daily_data.mean() > 0 else 0
            if weekly_cv > 0.2:
    ...
    def detect_performance_degradation(self, time_series: pd.Series, metric_name: str,
                                     window_size: int = 50) -> List[PatternInsight]:
        ...
        if any(keyword in metric_name.lower() for keyword in ['duration', 'time', 'latency', 'memory', 'cpu']):
            # Lower is better
            if recent_mean > historical_mean * 1.1:  # 10% worse
                is_degradation = True
                degradation_percent = ((recent_mean - historical_mean) / historical_mean) * 100
        else:
            # Higher is better (efficiency, coverage, etc.)
            if recent_mean < historical_mean * 0.9:  # 10% worse
                is_degradation = True
                degradation_percent = ((historical_mean - recent_mean) / historical_mean) * 100

        if is_degradation:
            severity = 'critical' if degradation_percent > 30 else 'high' if degradation_percent > 20 else 'medium'
            ...
            insight = PatternInsight(
                ...
                actionable_insights=self._generate_degradation_insights(metric_name, degradation_percent, severity),
                supporting_data={
    ...
    def _generate_degradation_insights(self, metric_name: str, degradation_percent: float, severity: str) -> List[str]:
        ...
        if severity == 'critical':
            insights.insert(0, f"URGENT: {degradation_percent:.1f}% degradation requires immediate attention")
...
class AIPerformanceAnalyzer:
    ...
    def __init__(self, metrics_dir: str = None):
        self.metrics_dir = Path(metrics_dir or os.path.join(os.path.expanduser("~"), ".claude", "metrics"))
    ...
    def analyze_metrics(self, metrics_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        analysis_results = {
            ...
            'data_summary': {
                ...
                'time_range': {
                    'start': metrics_data['timestamp'].min() if 'timestamp' in metrics_data.columns else None,
                    'end': metrics_data['timestamp'].max() if 'timestamp' in metrics_data.columns else None
                },
        ...
        try:
            ...
            recommendations = self._generate_recommendations(anomalies, predictions, patterns)
    ...
    def _detect_anomalies(self, metrics_data: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        for col in numeric_columns:
            for idx, value in metrics_data[col].items():
                if pd.notna(value):
                    is_anomalous, z_score = self.baseline_calculator.is_anomalous(col, value)
                    ...
                    if is_anomalous:
                        ...
                        anomaly = PerformanceAnomaly(
                            ...
                            root_cause_analysis=self._analyze_anomaly_root_cause(col, value, metrics_data.iloc[idx]),
                            recommendations=self._generate_anomaly_recommendations(col, value, z_score),
            ...
            try:
                ...
                for idx in ml_anomalies:
                    if idx < len(metrics_data):
                        ...
                        anomaly_score = anomaly_scores[idx] if idx < len(anomaly_scores) else 0.5
                        ...
                        if len(numeric_values) > 0:
                            ...
                            if max_deviation_metric:
                                anomaly = PerformanceAnomaly(
                                    ...
                                    root_cause_analysis=self._analyze_anomaly_root_cause(max_deviation_metric, row[max_deviation_metric], row),
                                    recommendations=self._generate_anomaly_recommendations(max_deviation_metric, row[max_deviation_metric], anomaly_score),
    ...
    def _analyze_anomaly_root_cause(self, metric_name: str, value: float, row: pd.Series) -> Dict[str, Any]:
    ...
    def _generate_anomaly_recommendations(self, metric_name: str, value: float, severity_score: float) -> List[str]:
        """Generate recommendations for addressing anomalies"""
        recommendations = []

        if severity_score > 2.5:  # High severity
            recommendations.append(f"URGENT: Investigate {metric_name} anomaly immediately")
            ...
            if metric in numeric_columns and len(metrics_data[metric].dropna()) >= 50:
                try:
                    ...
                    if metric not in self.performance_predictor.predictors:
                        ...
                        print(f"Trained predictor for {metric}: R² = {training_result.get('r2', 0):.3f}")
    ...
    def _analyze_patterns(self, metrics_data: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if 'timestamp' in metrics_data.columns:
                ...
                for col in numeric_columns:
                    if len(metrics_data[col].dropna()) >= 168:  # At least a week of data
                        time_series = pd.Series(
                            metrics_data[col].values,
                            index=metrics_data['timestamp']
                        ).dropna()

                        seasonal_patterns = self.pattern_analyzer.detect_seasonal_patterns(time_series, col)
                        patterns.extend([asdict(p) for p in seasonal_patterns])

                        degradation_patterns = self.pattern_analyzer.detect_performance_degradation(time_series, col)
    ...
    def _generate_recommendations(self, anomalies: List[Dict], predictions: List[Dict],
        ...
        if critical_anomalies:
            recommendations.append(f"🚨 CRITICAL: {len(critical_anomalies)} critical anomalies detected - immediate investigation required")

        # Prediction-based recommendations
        degrading_predictions = [p for p in predictions if p.get('trend_analysis', {}).get('trend') == 'increasing'
                               and any(keyword in p.get('metric_name', '') for keyword in ['duration', 'memory', 'cpu'])]
        if degrading_predictions:
            recommendations.append(f"📈 Performance degradation predicted for {len(degrading_predictions)} metrics - proactive optimization recommended")

        # Pattern-based recommendations
        degradation_patterns = [p for p in patterns if p.get('pattern_type') == 'degradation']
        if degradation_patterns:
            recommendations.append(f"📉 {len(degradation_patterns)} degradation patterns identified - review recent changes")

        correlation_patterns = [p for p in patterns if p.get('pattern_type') == 'correlation' and p.get('strength', 0) > 0.8]
        if correlation_patterns:
            recommendations.append(f"🔗 Strong correlations found - consider optimizing related metrics together")

        # General optimization recommendations
        if len(anomalies) > 5:
            recommendations.append("🔧 Multiple anomalies suggest systematic performance issues - comprehensive optimization needed")

        if not recommendations:
            recommendations.append("✅ Performance appears stable - continue current practices")
    ...
    def _save_insights(self, analysis_results: Dict[str, Any]):
        ...
        insight_entry = {
            ...
            'key_insights': analysis_results.get('recommendations', [])[:3]  # Top 3 recommendations
        }
    ...
    def _generate_executive_summary(self, analysis_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        critical_issues = len([a for a in analysis_results.get('anomalies', []) if a.get('severity') == 'critical'])
        ...
        return {
            ...
            'confidence_level': 'high' if len(analysis_results.get('anomalies', [])) > 10 else 'medium'
        }
    ...
    def _identify_optimization_opportunities(self, analysis_results: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for pattern in analysis_results.get('patterns', []):
            if pattern.get('pattern_type') == 'correlation' and pattern.get('strength', 0) > 0.8:
                opportunities.append({
    ...
    def _generate_action_items(self, analysis_results: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        critical_anomalies = [a for a in analysis_results.get('anomalies', []) if a.get('severity') == 'critical']
        ...
        degradation_patterns = [p for p in analysis_results.get('patterns', []) if p.get('pattern_type') == 'degradation']
...
def analyze_performance_data(metrics_file: str, metrics_dir: str = None) -> Dict[str, Any]:
...
if __name__ == "__main__":
    ...
    parser.add_argument('command', choices=['analyze', 'train', 'predict', 'report'],
    ...
    if np is None:
        print("Error: Required ML libraries not available. Install with: pip install numpy pandas scikit-learn")
        sys.exit(1)
