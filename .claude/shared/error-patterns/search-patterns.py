...
class ErrorPatternSearcher:
    ...
    def search_by_language(self, language: str) -> List[Dict]:
        ...
        return [p for p in self.patterns if language_lower in [l.lower() for l in p.get('languages', [])]]
...
def main():
    ...
    # Apply filters
    if args.category:
        results = [p for p in results if p.get('category', '').lower() == args.category.lower()]
    
    if args.language:
        results = [p for p in results if args.language.lower() in [l.lower() for l in p.get('languages', [])]]
    
    if args.severity:
        results = [p for p in results if p.get('severity', '').lower() == args.severity.lower()]
