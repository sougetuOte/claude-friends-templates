...
"""
        ...
        # 重要度別に問題を分類
        critical_items = [item for item in self.drift_items if item['severity'] == 'critical']
        high_items = [item for item in self.drift_items if item['severity'] == 'high']
        medium_items = [item for item in self.drift_items if item['severity'] == 'medium']
...
def main():
    ...
    parser.add_argument("--design-paths", nargs='+', 
                       default=["docs/design", ".claude/shared/templates/design"],
                       help="設計ドキュメントのパス")
    parser.add_argument("--src-paths", nargs='+', 
                       default=["src", "lib", "app"],
                       help="ソースコードのパス")
