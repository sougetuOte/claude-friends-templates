...
class QualityChecker:
    ...
    def load_config(self, config_path):
        ...
        default_config = {
            "thresholds": {
                "coverage": {"global": 80, "new_code": 90, "per_file": 60},
                "complexity": {"cyclomatic": 10, "cognitive": 15},
                "duplication": 5,
                "security": {"critical": 0, "high": 0, "medium": 3, "low": 10},
            },
            "exclude": [
                "**/*.test.js",
                "**/*.spec.ts",
            ...
            "rules": {"enforce_tdd": True, "require_docs": True, "strict_typing": True},
        }
    ...
    def check_test_coverage(self):
        ...
        coverage_data = {"line": 0, "branch": 0, "function": 0, "files": {}}
        ...
        if os.path.exists("setup.py") or os.path.exists("pyproject.toml"):
            try:
                ...
                if result.returncode == 0:
                    ...
                    coverage_data["line"] = cov_json.get("totals", {}).get(
                        "percent_covered", 0
                    )
                    ...
                    for file, data in cov_json.get("files", {}).items():
                        if not self.should_exclude(file):
                            coverage_data["files"][file] = data.get("summary", {}).get(
                                "percent_covered", 0
                            )
        ...
        if global_coverage < threshold * COVERAGE_YELLOW_THRESHOLD:  # 70%
            ...
            self.add_issue(
                "coverage",
                f"カバレッジが低すぎます: {global_coverage:.1f}%",
                "critical",
            )
            ...
            self.add_issue(
                "coverage", f"カバレッジが目標未達: {global_coverage:.1f}%", "high"
            )
    ...
    def check_code_complexity(self):
        ...
        complexity_data = {"average": 0, "max": 0, "files_exceeded": []}
        ...
        for file in py_files:
            if not self.should_exclude(str(file)):
                ...
                if complexity > 0:
                    ...
                    if (
                        complexity
                        > self.config["thresholds"]["complexity"]["cyclomatic"]
                    ):
                        complexity_data["files_exceeded"].append(
                            {"file": str(file), "complexity": complexity}
                        )
        ...
        if (
            complexity_data["max"]
            > self.config["thresholds"]["complexity"]["cyclomatic"] * 1.5
        ):
            self.add_issue(
                "complexity",
    ...
    def check_security(self):
        ...
        for root, dirs, files in os.walk("."):
            ...
            for file in files:
                ...
                if file.endswith(
                    (".py", ".js", ".ts", ".java")
                ) and not self.should_exclude(file_path):
                    try:
                        with open(file_path, "r", encoding="utf-8") as f:
                            content = f.read()
    ...
    def check_duplication(self):
        ...
        duplication_data = {"percentage": 0, "duplicates": []}
        ...
        for root, dirs, files in os.walk("."):
            ...
            for file in files:
                file_path = os.path.join(root, file)
                if file.endswith((".py", ".js", ".ts")) and not self.should_exclude(
                    file_path
                ):
                    try:
                        with open(file_path, "r", encoding="utf-8") as f:
...
"""
        ...
        if "coverage" in self.results["metrics"]:
            ...
            emoji = (
                "✅" if cov >= self.config["thresholds"]["coverage"]["global"] else "❌"
            )
        ...
        if "complexity" in self.results["metrics"]:
            ...
            emoji = (
                "✅"
                if comp["max"] <= self.config["thresholds"]["complexity"]["cyclomatic"]
                else "❌"
            )
        ...
        if self.results["issues"]:
            ...
            for severity in ["critical", "high", "medium", "low"]:
                issues = [
                    i for i in self.results["issues"] if i["severity"] == severity
                ]
                if issues:
...
def main():
    ...
    parser.add_argument(
        "--full", action="store_true", help="フルチェック（すべての項目）"
    )
    parser.add_argument(
        "--ci", action="store_true", help="CI用（エラー時に非ゼロ終了）"
    )
    parser.add_argument(
        "--format",
