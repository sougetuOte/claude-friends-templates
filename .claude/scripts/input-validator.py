...
class InputValidator:
    ...
    def validate_input(self, user_input: str, context: str = "") -> ValidationResult:
        ...
        if input_hash in self.validation_cache:
            ...
            self.logger.info(f"Using cached validation result for input hash: {input_hash[:8]}")
        ...
        for pattern_info in self.blocked_patterns:
            if re.search(pattern_info["pattern"], user_input, re.IGNORECASE | re.MULTILINE | re.DOTALL):
    ...
    def strict_sanitize(self, text: str) -> str:
        ...
        # スクリプト関連の除去
        text = re.sub(r'(javascript|vbscript|onload|onerror|onclick):', '', text, flags=re.IGNORECASE)

        # SQLキーワードの除去
        sql_keywords = ['DROP', 'DELETE', 'UPDATE', 'INSERT', 'SELECT', 'UNION', 'ALTER']
    ...
    def validate_command_input(self, command: str, args: List[str] = None) -> ValidationResult:
        """コマンド入力の特別な検証"""
    ...
    def generate_security_report(self) -> Dict:
        ...
        high_risk_violations = len([v for v in violations if v.get('risk_score', 0) > 0.7])
